...
async function sendCommand(commandArray) {
    if (writer) {
        // Copy all elements except the last two bytes reserved for CRC
        const commandPayload = commandArray.slice(0, commandArray.length - 2);
        const dataString = String.fromCharCode(...commandPayload);
        const crc = crc16(dataString);

        // Convert CRC to two bytes and replace the last two bytes of the original commandArray
